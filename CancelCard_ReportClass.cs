...
// Type: AMS.CancelCard_ReportClass
...
namespace AMS
{
  internal class CancelCard_ReportClass
  {
    ...
    public cancelcard cancelcard1;

    public CancelCard_ReportClass()
    {
    ...
    private void InitializeComponent()
    {
      ...
      this.cancelcard1 = new cancelcard();
    }

    public cancelcard CHECK_CANCELCARD(
    ...
    public cancelcard Check_Report_All(string para_ReportType) => this.cancelcard1;

    public cancelcard Check_Report_Individual(string para_ReportType, string para_BuildingNo)
    {
      ((ReportDocument) this.cancelcard1).SetDataSource(ModDoorLockSystem.Class_ModDoorLockSystem_Object_Ptr.tempDataSet.DTransactions.DataSet);
      ...
      if (CancelCard_ReportClass.Temp_MDB_SaveFlag)
      ...
      localCommand1.CommandText = localCommand1.CommandText + "SELECT * FROM CardTrans in '" + Application.UserAppDataPath + "\\BuildingDB" + para_BuildingNo + ".mdb 'Where ((CardTrans.TransType)='CC')AND Left(CardTrans.RoomNo,1)=  '" + ModDoorLockSystem.Class_ModDoorLockSystem_Object_Ptr.sBldNo + "'";
      ...
      ((ReportDocument) this.cancelcard1).DataDefinition.FormulaFields[2].Text = "\"" + ModDoorLockSystem.Class_ModDoorLockSystem_Object_Ptr.gUSERID + "\"";
      ((ReportDocument) this.cancelcard1).DataDefinition.FormulaFields[1].Text = "\"" + DateTime.Now.ToShortDateString() + " " + DateTime.Now.ToShortTimeString() + "\"";
      ((ReportDocument) this.cancelcard1).DataDefinition.FormulaFields[0].Text = "\"" + ModDoorLockSystem.Class_ModDoorLockSystem_Object_Ptr.gDescription + "\"";
      ((ReportDocument) this.cancelcard1).DataDefinition.FormulaFields[3].Text = "\"" + para_ReportType + "\"";
      ((ReportDocument) this.cancelcard1).DataDefinition.FormulaFields[5].Text = "\"" + Global_Data.sBuildingName + "\"";
      return this.cancelcard1;
    }
  }
}
