...
// Type: AMS.CancelWoCard_ReportClass
...
namespace AMS
{
  internal class CancelWoCard_ReportClass
  {
    ...
    public cancelwocard cancelwocard1;

    public CancelWoCard_ReportClass()
    {
    ...
    private void InitializeComponent()
    {
      ...
      this.cancelwocard1 = new cancelwocard();
    }

    public cancelwocard CHECK_CANCELWITHOUTCARD(
    ...
    public cancelwocard Check_Report_All(string para_ReportType) => this.cancelwocard1;

    public cancelwocard Check_Report_Individual(string para_ReportType, string para_BuildingNo)
    {
      ((ReportDocument) this.cancelwocard1).SetDataSource(ModDoorLockSystem.Class_ModDoorLockSystem_Object_Ptr.tempDataSet.DTransactions.DataSet);
      ...
      if (CancelWoCard_ReportClass.Temp_MDB_SaveFlag)
      ...
      localCommand1.CommandText = localCommand1.CommandText + "SELECT * FROM CardTrans in '" + Application.UserAppDataPath + "\\BuildingDB" + para_BuildingNo + ".mdb 'Where ((CardTrans.TransType)='CN')AND Left(CardTrans.RoomNo,1)=  '" + ModDoorLockSystem.Class_ModDoorLockSystem_Object_Ptr.sBldNo + "'";
      ...
      ((ReportDocument) this.cancelwocard1).DataDefinition.FormulaFields[2].Text = "\"" + ModDoorLockSystem.Class_ModDoorLockSystem_Object_Ptr.gUSERID + "\"";
      ((ReportDocument) this.cancelwocard1).DataDefinition.FormulaFields[1].Text = "\"" + DateTime.Now.ToShortDateString() + " " + DateTime.Now.ToShortTimeString() + "\"";
      ((ReportDocument) this.cancelwocard1).DataDefinition.FormulaFields[0].Text = "\"" + ModDoorLockSystem.Class_ModDoorLockSystem_Object_Ptr.gDescription + "\"";
      ((ReportDocument) this.cancelwocard1).DataDefinition.FormulaFields[3].Text = "\"" + para_ReportType + "\"";
      ((ReportDocument) this.cancelwocard1).DataDefinition.FormulaFields[5].Text = "\"" + Global_Data.sBuildingName + "\"";
      return this.cancelwocard1;
    }
  }
}
