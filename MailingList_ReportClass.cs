...
// Type: AMS.MailingList_ReportClass
...
namespace AMS
{
  internal class MailingList_ReportClass
  {
    ...
    public accmailinglist accmailinglist1;

    public MailingList_ReportClass()
    {
    ...
    private void InitializeComponent()
    {
      ...
      this.accmailinglist1 = new accmailinglist();
    }

    public accmailinglist CHECK_MAILINGLIST(bool para_bCurrent_Flag)
    {
      return para_bCurrent_Flag ? this.Check_GuestList_Report_Individual() : this.Check_GuestList_Report_All();
    }

    public accmailinglist Check_GuestList_Report_All()
    {
      ((ReportDocument) this.accmailinglist1).SetDataSource(ModDoorLockSystem.Class_ModDoorLockSystem_Object_Ptr.tempDataSet.AccGuestList.DataSet);
      ...
      if (MailingList_ReportClass.Temp_MDB_SaveFlag)
      ...
      if (MailingList_ReportClass.Temp_MDB_SaveFlag)
        ModDoorLockSystem.Class_ModDoorLockSystem_Object_Ptr.accGuestListTableAdapter.Fill(ModDoorLockSystem.Class_ModDoorLockSystem_Object_Ptr.tempDataSet.AccGuestList);
      ((ReportDocument) this.accmailinglist1).DataDefinition.FormulaFields[1].Text = "\"" + ModDoorLockSystem.Class_ModDoorLockSystem_Object_Ptr.gUSERID + "\"";
      ((ReportDocument) this.accmailinglist1).DataDefinition.FormulaFields[2].Text = "\"" + DateTime.Now.ToShortDateString() + " " + DateTime.Now.ToShortTimeString() + "\"";
      ((ReportDocument) this.accmailinglist1).DataDefinition.FormulaFields[0].Text = "\"" + ModDoorLockSystem.Class_ModDoorLockSystem_Object_Ptr.gDescription + "\"";
      ((ReportDocument) this.accmailinglist1).DataDefinition.FormulaFields[3].Text = "\"" + Global_Data.sBuildingName + "\"";
      return this.accmailinglist1;
    }

    public accmailinglist Check_GuestList_Report_Individual()
    {
      ((ReportDocument) this.accmailinglist1).SetDataSource(ModDoorLockSystem.Class_ModDoorLockSystem_Object_Ptr.tempDataSet.AccGuestList.DataSet);
      this.db.Local_Command.CommandText = "Delete * from AccGuestList";
      this.db.Local_Command.Connection = ModDoorLockSystem.Class_ModDoorLockSystem_Object_Ptr.accGuestListTableAdapter.Connection;
